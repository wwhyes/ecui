...
gulp.task('build:style', function() {
  var banner = [
    '/*!',
    ' * ECUI v<%= pkg.version %> (<%= pkg.homepage %>)',
    ' * Copyright <%= new Date().getFullYear() %>©深圳易仓科技有限公司.All Rights Reserved 粤ICP备13020851号',
    ' * Licensed under the <%= pkg.license %> license',
    ' */',
    ' '
  ].join('\n');
  gulp
    .src('src/style/ecui.less', option)
...
gulp.task('build:example:assets', function() {
  gulp
    .src('src/example/**/*.?(css|png|jpg|gif|js|mp4)', option)
    .pipe(gulp.dest(dist))
    .pipe(browserSync.reload({ stream: true }));
});
